...
const TEXTURES = {
  soleil:  "radial-gradient(circle at 30% 30%, #fff1b5, #ffb300 55%, #ff6d00 100%)",
...
function getBodyId() {
  ...
  const id = (url.searchParams.get("body") || "").toLowerCase();
...
function renderMini(bodies, activeId) {
  ...
  bodies.forEach(b => {
    ...
    if (b.id === "saturn") dot.classList.add("ring");
    ...
    dot.style.setProperty("--tex", TEXTURES[b.id] || TEXTURES.mercury);
    ...
    btn.addEventListener("click", () => {
      location.href = `planet.html?body=${encodeURIComponent(b.id)}`;
    });
...
function renderDetails(b) {
  ...
  if (b.id === "saturn") visual.classList.add("ring");
  visual.style.setProperty("--tex", TEXTURES[b.id] || TEXTURES.mercury);
